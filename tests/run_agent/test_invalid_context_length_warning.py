...
def test_string_k_suffix_context_length_works():
    """context_length: '256K' should parse as a human token count."""
    with patch("run_agent.logger") as mock_logger:
        agent = _build_agent({"default": "gpt5.4", "provider": "custom",
                              "base_url": "http://localhost:4000/v1",
                              "context_length": "256K"})
    ...
    for call in mock_logger.warning.call_args_list:
        assert "Invalid" not in str(call)
...
def test_custom_providers_k_suffix_context_length_works():
    """Human suffix context_length in custom_providers should parse."""
    custom_providers = [
        {
            "name": "LiteLLM",
            "base_url": "http://localhost:4000/v1",
